...
def xml_to_json_projects(xml_file_path):
    ...
    try:
        ...
        log_error(
            function_name="xml_to_json_projects",
            endpoint="process-hist-xml-files",
            log_message=f"XML validation error after correction {xml_file_path}: {str(e)}",
            error_type=type(e).__name__,
            stack_trace=stack_trace,
            json_payload=json.dumps({"filename": xml_file_path})
        )
        raise Exception(f"XML validation error after correction {xml_file_path}: {str(e)}")
        ...
        log_error(
            function_name="xml_to_json_projects",
            endpoint="process-hist-xml-files",
            log_message=f"Error parsing XML {xml_file_path}: {str(e)}",
            error_type=type(e).__name__,
            stack_trace=stack_trace,
            json_payload=json.dumps({"filename": xml_file_path})
        )
        raise Exception(f"Error parsing XML {xml_file_path}: {str(e)}")
        ...
        log_error(
            function_name="xml_to_json_projects",
            endpoint="process-hist-xml-files",
            log_message=f"An unexpected error occurred {xml_file_path}: {str(e)}",
            error_type=type(e).__name__,
            stack_trace=stack_trace,
            json_payload=json.dumps({"filename": xml_file_path})
        )
        raise Exception(f"An unexpected error occurred {xml_file_path}: {str(e)}")
...
def process_file(file_path, gcs_client, bucket, settings):
    ...
    if os.path.exists(xml_file):
            os.remove(xml_file)
            log_default(log_message=f"Deleted temp file: {xml_file}",
...
def push_to_bq(df, big_query_client, TABLE_ID,FILE_PATH , write='WRITE_APPEND'):
    ...
    Args:
        ...
        big_query_client (google.cloud.bigquery.Client): The BigQuery client instance.
        CC_FEED_TABLE_ID (str): The full table ID in the format `project.dataset.table`.
        FILE_PATH (str): The file path being processed, used for logging.
    ...
    Returns:
        int: The number of rows successfully loaded into BigQuery.
...
def insert_log(dataset, table, file_path, rows ,big_query_client, bucket, settings):
    ...
    query = f"""Insert into {dataset}.{table} (source,bucket,file_path, num_rows, status) values ('{source}','{bucket}','{file_path}', {rows}, 'Success')"""
    query_job = big_query_client.query_table(query)
    log_default(log_message=f"Inserted log for {file_path}",
                json_payload=json.dumps({"filename": file_path})
    )

def get_loaded_filelist(project_id, dataset_id, table_id, big_query_client):
    ...
    """
    query_job = big_query_client.query_table(query)
    return [row['file_path'] for row in query_job]
