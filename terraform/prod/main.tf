...
# Create a Service Account for the Workflow
  resource "google_service_account" "workflow_service_account" {
    ...
    project      = var.project_id
    depends_on = [ module.enable_apis_prod ]
  }
...
resource "google_bigquery_dataset" "sales_recommender_prod" {
  dataset_id = "sales_recommender_prod"
  project    = var.project_id
  location   = "us-west1"
  depends_on = [ module.enable_apis_prod ]
...
# Enabling APIs for the prod project
module "enable_apis_prod" {
